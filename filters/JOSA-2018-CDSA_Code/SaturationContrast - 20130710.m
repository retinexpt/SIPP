...
%         win=5;
%         bmax=MedianFilter(maxpic,1);
%         amax=MedianFilter(ehcmax,1);
%         max1=getlocalmax(bmax,win);
%         min1=getlocalmin(bmax,win);
%         max2=getlocalmax(amax,win);
%         min2=getlocalmin(amax,win);
%         delta1=(max1-min1)./(max1+~max1);
%         delta2=(max2-min2)./(max2+~max2); 
%         climit=0.2; %contrast limit---不能太小也不能太大
%         tag=delta1<climit;
...
%         win=5;
%         mean1=getlocalmean(maxpic,win);
%         delta1=abs(mean1-maxpic);
%         delta1=getlocalmean(delta1,win)./(mean1+~mean1);
%         mean2=getlocalmean(ehcmax,win);
%         delta2=abs(mean2-ehcmax);
%         delta2=getlocalmean(delta2,win)./(mean2+~mean2);
%         climit=0.05; %contrast limit---不能太小也不能太大
%         tag=delta1<climit;
%         ratio=(delta2+(climit-delta1).*tag)./(delta1.*(~tag)+climit.*tag);

        blkwin=30;%消去artifacts
        ratio=getspecialmean(ratio,tpic,blkwin);
        pichsv(:,:,3)=ehcmax/255;
