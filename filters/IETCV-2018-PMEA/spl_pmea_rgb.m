function varargout = spl_pmea_rgb(option, imgSrc, imgName,varargin)
% Interface function
% *************************
...
if strcmpi(option,'ui_name')==1
	varargout{1} = 'IET CV\2018_PMEA_RGB';
	return;
end
...
if strcmpi(option,'run')==1
	tic;	
    imgOut = pmea_rgb(imgSrc);
	elap_time = toc;
    
    title = sprintf('%s - %f s', 'PMEA-rgb',elap_time);
